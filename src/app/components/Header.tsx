        ...
        <div className="flex items-center justify-between h-20">
          ...
          >
            ...
            <div className="hidden sm:block">
              <div className={`transition-colors font-semibold ${isScrolled ? 'text-gray-900' : 'text-white'}`}>
                Tempo Holdings
              </div>
              <div className={`text-xs transition-colors ${isScrolled ? 'text-blue-600' : 'text-blue-200'}`}>
                Marketing Solutions
              </div>
            </div>
          </button>
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {['Home', 'About', 'Services', 'Work', 'Clients'].map((item) => (
              <button
                key={item}
                onClick={() => scrollToSection(item === 'Home' ? 'hero' : item === 'Work' ? 'portfolio' : item === 'Clients' ? 'testimonials' : item.toLowerCase())}
